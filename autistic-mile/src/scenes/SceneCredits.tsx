...
export const SceneCredits: React.FC = () => {
  ...
  const scanX = interpolate(frame % 50, [0, 50], [-80, 2080]);
  ...
  return (
    <div style={{ position: "absolute", inset: 0, background: "#1a2744", overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", left: scanX, top: 0, bottom: 0, width: 2, background: "linear-gradient(180deg, transparent, rgba(90,174,224,0.14), transparent)", pointerEvents: "none" }} />
      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-end", paddingBottom: 160, gap: 18 }}>
        <div style={{ fontSize: 38, fontWeight: 400, color: "#FFFFFF", letterSpacing: "0.06em", fontFamily: '"Inter", "Segoe UI", sans-serif', opacity: line1, transform: `translateY(${float1}px)` }}>Hosted by Jim Gaffney</div>
        <div style={{ fontSize: 26, fontWeight: 300, color: "#f5e6c8", letterSpacing: "0.04em", fontFamily: '"Inter", "Segoe UI", sans-serif', opacity: line2, transform: `translateY(${float2}px)` }}>In collaboration with Jacksonville School for Autism</div>
      </div>
