...
export const SceneCredits: React.FC = () => {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, background: "#1a2744", overflow: "hidden" }}>
      ...
      {([{ left: 120, bottom: 90, size: 9, color: "#5aaee0", phase: 0 }, { left: 150, bottom: 70, size: 6, color: "#f5e6c8", phase: 1.2 }, { right: 120, bottom: 90, size: 9, color: "#5aaee0", phase: 0.6 }, { right: 150, bottom: 70, size: 6, color: "#f5e6c8", phase: 1.8 }] as any[]).map((d, i) => (
        <div key={i} style={{ position: "absolute", left: d.left, right: d.right, bottom: d.bottom + Math.sin(frame * 0.06 + d.phase) * 8, width: d.size, height: d.size, borderRadius: "50%", background: d.color, opacity: interpolate(frame, [14, 24], [0, 0.7], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }) }} />
      ))}

      {/* ENTER TRANSITION — slabs slide out to reveal the scene (mirror of tagline exit) */}
      {Array.from({ length: N_SLABS }, (_, i) => {
        const toRight = i % 2 === 0;
        // Stagger matches exit stagger; all gone by frame 13
        const start = i * 2;
        const end   = start + 10;
        const p = interpolate(frame, [start, end], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
        const tx = toRight
          ? interpolate(p, [0, 1], [0, 1920])
          : interpolate(p, [0, 1], [0, -1920]);
        return (
          <div key={i} style={{
            position: "absolute",
