...
export const SceneLogo: React.FC = () => {
  ...
  const breathe = 1 + Math.sin(frame * 0.12) * 0.022;
  ...
  return (
    <div style={{ position: "absolute", inset: 0, background: "#1a2744", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ position: "absolute", inset: 0, background: `radial-gradient(ellipse ${glowW}px ${glowH}px at 50% 50%, rgba(90,174,224,${glowOpacity}) 0%, transparent 65%)` }} />
      <div style={{ position: "absolute", width: 420, height: 420, borderRadius: "50%", border: "2px solid rgba(90,174,224,0.6)", transform: `scale(${ringScale})`, opacity: ringOpacity, pointerEvents: "none" }} />
      {DOT_ORBS.map((orb, i) => {
        const rad = (orb.angle * Math.PI) / 180;
        const floatY = Math.sin(frame * 0.07 + orb.phase) * 12;
        const floatX = Math.cos(frame * 0.07 + orb.phase) * 6;
        return (
          <div key={i} style={{ position: "absolute", left: `calc(50% + ${Math.cos(rad) * orb.dist + floatX}px - ${orb.size / 2}px)`, top: `calc(50% + ${Math.sin(rad) * orb.dist + floatY}px - ${orb.size / 2}px)`, width: orb.size, height: orb.size, borderRadius: "50%", background: orb.color, opacity: dotsIn * 0.75 }} />
        );
      })}
      <div style={{ opacity: enter, transform: `scale(${interpolate(enter, [0, 1], [0.82, breathe])})` }}>
        <TAMLogo size={400} />
      </div>
    </div>
  );
